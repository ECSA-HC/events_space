...
async function deleteLog(id) {
  ...
  try {
    ...
    if (selectedLog.value?.id === id) closeDetail()
  } catch (e) {
...
async function confirmClearAll() {
  if (!confirm(`Delete all ${logs.value.length} log entries? This cannot be undone.`)) return
  const ids = [...logs.value.map(l => l.id)]
  for (const id of ids) {
    try { await api.delete(`/email-logs/${id}`) } catch (_) {}
  }
  logs.value = []
  total.value = 0
  closeDetail()
