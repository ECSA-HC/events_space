...
def export_abstracts_pdf(
    ...
    def on_cover(c, doc):
        ...
        # Top strip — off-white so the logos (designed for a light background)
        # show up clearly on their own, no chip needed.
        c.setFillColor(Color(0.96, 0.96, 0.94))
        c.rect(0, H - 3.8 * cm, W, 3.8 * cm, fill=1, stroke=0)
        # Logos (Eswatini MoH left, ECSA-HC right) — pinned to the top
        # corners, clear of the heading text below.
        logo_h = 1.6 * cm
    ...
    if programme_path.exists():
        ...
        # Read the generated abstract book
        generated_pdf = PdfReader(buf)
        total_generated = len(generated_pdf.pages)
